...
実際のファイル名には `LoCAL-0.3.0-` が付きます。署名鍵を設定したAndroidビルドでは末尾が `release.apk` になります。
...
Windowsの画面を見せるには、両方をペアリング後、Windows側の「画面共有」で画面と相手を選び「共有を申し込む」を押します。WindowsまたはAndroidの受け側で「画面を見る」を承認し、画面セッションの「表示する」を押します。端末のWebViewに対応するH.264 WebCodecsデコーダーが必要です。閲覧専用で、音声・遠隔操作はありません。終了するときは「表示を終了」を押します。この機能はプレビューで、Androidを含む実機2台での描画・遅延確認は未実施です。Androidから画面を送信する機能はまだありません。

APKはActionsで生成した開発署名付きです。ビルドごとに開発鍵が変わるため、以前の開発署名APKから更新する場合は再インストールが必要です。Androidの受信ファイルは先に書き出してください。Windows / macOSは商用コード署名・公証をしていません。
...
「ファイルを送る」から1～8件を選択 → 相手の「ファイル転送」で各ファイルの「受信する」を押します。受信後にBLAKE3を検証し、完了したファイルのみ保存先に公開します。名前にUUIDを付けて、既存ファイルを上書きしません。ほかの転送が進行中なら、同時に実行できるのは合計8件までです。
...
ペアリング済み端末の「通話」、または通話画面の「電話をかける」を押します。着信側が「応答」を押して初めてマイクを開始します。通話中はミュートと終了ができます。Androidでは初回にマイク権限を許可してください。macOS / WindowsでもOSのマイク許可が必要です。

呼び出し情報はペアリング済みQUIC接続で暗号化し、音声はWebRTCでLAN内を直接流します。STUN / TURNやインターネット中継は使わないため、同じLANでもAP isolation・VPN・複雑なVLAN構成では音声経路を確立できない場合があります。現在は1対1の音声のみで、ビデオ通話・グループ通話・電話番号への発信には対応していません。

## 接続できない

- 相手の「設定」にあるIPv4アドレスとポートを「IPアドレスで接続」に入力。各アドレスの「コピー」で共有できます。例 `192.168.1.20:53319`。
