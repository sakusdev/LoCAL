...
実際のファイル名には `LoCAL-0.1.0-` が付きます。署名鍵を設定したAndroidビルドでは末尾が `release.apk` になります。
...
初期APKは開発署名です。異なる開発署名のAPKへ更新すると再インストールが必要になる場合があるため、Androidの受信ファイルは先に書き出してください。Windows / macOSは商用コード署名・公証をしていません。
...
Androidは受信ファイルをアプリ内に保存します。「端末に保存…」でDownloadsなどへ書き出してください。アプリの削除やデータ消去で、書き出していない受信ファイルと端末の鍵・履歴が消えます。
