...
画面を見せるには、両方をペアリング後、WindowsまたはAndroid側の「画面共有」で画面と相手を選び「共有を申し込む」を押します。受け側で「画面を見る」を承認し、画面セッションの「表示する」を押します。Androidから送る場合は、相手の承認後にAndroid標準の画面共有許可が表示されます。共有中は常駐通知からいつでも停止できます。端末のWebViewに対応するH.264 WebCodecsデコーダーが必要です。閲覧専用で、音声・遠隔操作はありません。この機能はプレビューで、Androidを含む実機2台での描画・遅延確認は未実施です。
...
「メッセージ」で入力して送信。Ctrl/Cmd+Enterでも送信できます。「クリップボードを貼り付け」は下書きに貼り付け、送信ボタンを押して初めて相手へ送ります。受け取った内容の「コピー」で自分のクリップボードに保存します。

「クリップボード」で同期する端末を選び、「同期を有効にする」をオンにすると、その後コピーしたテキストが選択端末へ送られます。開始前の履歴やクリップボードは自動送信しません。受信内容をクリップボードへ書いた後の再送も抑止します。同期はLoCALを画面に開いている間だけ動作し、画像・ファイル・16 KiBを超えるテキストは対象外です。パスワードなどをコピーする前は同期を停止してください。
