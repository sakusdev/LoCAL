...
Android registers H.264 encode/decode roles after its bundled UI confirms WebCodecs availability. Incoming frames use the bounded QUIC video queue. Outgoing capture requires receiver acceptance followed by Android's MediaProjection consent; MediaCodec supplies Annex B H.264 access units to the same authenticated screen stream. Remote pointer/keyboard events and system-audio media transport are **not** implemented; their capabilities remain independently negotiated. See [`SCREEN_SHARING.md`](SCREEN_SHARING.md) for capture backends and permission separation.

## Storage and future work

`identity.json` has mode 0600 on Unix, its data directory 0700. A process lock prevents simultaneous use. SQLite WAL stores trusted IDs, name and messages; UI shows latest 200 messages. File content is separate. Future channels include sensors, clipboard images and custom channels; v1 does not silently accept them.
